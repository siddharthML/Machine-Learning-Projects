...
library(mlr)
#Training a learner means fitting a model to a given data set. In mlr this can be done by calling a function train on a Learner and a suitable Task.

# We start with a classification example and perform a linear discriminant analysis on iris data set.
...
#The fitted model is in slot $learner.model of the WrappedModel object can be accessed using **getLearnerModel**

#In the following example we cluster the RUspini data set (which has four groups and two features) by K-means with K=4 and extract the output underlying kmeans function.
...
#By default, the whole data set in the Task is used for training.The subset argument of train takes logical or integer vector that indicates which observation to use, for example if you want to split your data intro training and a test set or if you want to fit separate models to different subgroups of data.
...
#Use 1/3 of the observation for training
